...
<style>
/* =======================================================
   Figer l'en-tête et le pied des tableaux principaux
   ======================================================= */
...
#viewTabsContent .table-responsive tfoot td { position: sticky; bottom: 0; z-index: 100; outline: 1px solid rgba(255,255,255,0.05); }
</style>
...
<div class="tab-content bg-white border border-top-0 p-3 rounded-bottom shadow-sm" id="viewTabsContent">
    ...
    <div class="tab-pane <?= !$isManager ? 'active' : '' ?>" id="vue-detail" role="tabpanel">
        <?php if ($isManager): ?>
        <div class="d-flex justify-content-between align-items-center mb-4 bg-light p-2 rounded border">
            ...
            <button type="button" class="btn btn-sm btn-outline-secondary" id="btnToggleDetailChart" onclick="toggleDashboardSection('detail_chart_container', 'cookie_detail_chart')">
                <i class="bi bi-eye-slash"></i> Masquer Graphique
            </button>
        </div>
        
        <div id="detail_chart_container" class="bg-light p-3 rounded shadow-sm border mb-4">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover align-middle mb-0">
                ...
                <tfoot class="table-dark text-center">
                    <tr>
                        <td class="text-end fw-bold align-middle"><?= $isManager ? 'TOTAL AFFECTÉ (Ce profil)' : 'MON TOTAL AFFECTÉ' ?></td>
                        <?php foreach($dash_months as $m_key => $m_data): 
    ...
    <div class="tab-pane" id="vue-graph-user" role="tabpanel">
        ...
        <div class="bg-light p-3 rounded shadow-sm border mb-4">
            ...
            <canvas id="detailChartUser" style="max-height: 400px; width: 100%;"></canvas>
        </div>
    </div>
    <?php endif; ?>
...
function toggleDashboardSection(containerId, cookieName) {
    ...
    let btnId = '';
    let label = 'Graphique';
    if (containerId === 'kpi_container') { btnId = 'btnToggleKpi'; label = 'KPIs'; }
    else if (containerId === 'chart_container') { btnId = 'btnToggleChart'; }
    else if (containerId === 'detail_chart_container') { btnId = 'btnToggleDetailChart'; }
    
    const btn = document.getElementById(btnId);
    
    if (isHidden) {
        container.classList.remove('d-none'); setCookie(cookieName, 'show', 365);
        if (btn) btn.innerHTML = '<i class="bi bi-eye-slash"></i> Masquer ' + label;
        window.dispatchEvent(new Event('resize'));
    } else {
        container.classList.add('d-none'); setCookie(cookieName, 'hide', 365);
        if (btn) btn.innerHTML = '<i class="bi bi-eye"></i> Afficher ' + label;
    }
}
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const detailChartContainer = document.getElementById('detail_chart_container');
    ...
    if (detailChartContainer && getCookie('cookie_detail_chart') === 'hide') {
        detailChartContainer.classList.add('d-none');
        const btnDetailChart = document.getElementById('btnToggleDetailChart'); if (btnDetailChart) btnDetailChart.innerHTML = '<i class="bi bi-eye"></i> Afficher Graphique';
    }
    ...
    const canvasId = document.getElementById('detailChart') ? 'detailChart' : 'detailChartUser';
    const ctxDetail = document.getElementById(canvasId);
