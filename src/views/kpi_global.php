<?php
$kpi_total_cap = 0;
foreach($dash_months as $m) { $kpi_total_cap += $m['working_days']; }
$kpi_total_cap *= $real_users_count;

$kpi_total_load = 0; $kpi_total_unassigned = 0;
foreach($dash_months as $m_key => $m_data) {
    foreach($displayUsers as $uid => $uname) {
        $val = $pivot_user_month[$uid][$m_key] ?? 0;
        if ($uid === '_virtual_unassigned_') { $kpi_total_unassigned += $val; }
        else { $kpi_total_load += $val; }
    }
}
...
$charge_color = 'text-success';
if ($kpi_taux_charge < 70) $charge_color = 'text-info';
if ($kpi_taux_charge > 95) $charge_color = 'text-warning';
if ($kpi_taux_charge > 100) $charge_color = 'text-danger';

// EVOLUTION: Ajustement dynamique de la grille Bootstrap (4 colonnes ou 3 colonnes)
$grid_class = $show_backlog ? 'col-md-3' : 'col-md-4';
?>
...
<div class="row g-3 mb-4">
    <div class="<?= $grid_class ?>">
        <div class="card border-0 shadow-sm h-100" style="border-left: 4px solid #3b82f6 !important; border-radius: 8px;">
            <div class="card-body p-3 d-flex align-items-center">
                <div class="bg-primary bg-opacity-10 p-3 rounded-circle me-3 text-primary d-flex align-items-center justify-content-center" style="width: 48px; height: 48px;"><i class="bi bi-speedometer2 fs-5"></i></div>
                <div>
    ...
    <div class="<?= $grid_class ?>">
        <div class="card border-0 shadow-sm h-100" style="border-left: 4px solid #f59e0b !important; border-radius: 8px;">
            <div class="card-body p-3 d-flex align-items-center">
                <div class="bg-warning bg-opacity-10 p-3 rounded-circle me-3 text-warning d-flex align-items-center justify-content-center" style="width: 48px; height: 48px;"><i class="bi bi-inbox-fill fs-5"></i></div>
                <div>
    ...
    <div class="<?= $grid_class ?>">
        <div class="card border-0 shadow-sm h-100" style="border-left: 4px solid #10b981 !important; border-radius: 8px;">
            <div class="card-body p-3 d-flex align-items-center">
                <div class="bg-success bg-opacity-10 p-3 rounded-circle me-3 text-success d-flex align-items-center justify-content-center" style="width: 48px; height: 48px;"><i class="bi bi-rocket-takeoff-fill fs-5"></i></div>
                <div>
    ...
    <div class="<?= $grid_class ?>">
        <div class="card border-0 shadow-sm h-100" style="border-left: 4px solid #8b5cf6 !important; border-radius: 8px;">
            <div class="card-body p-3 d-flex align-items-center">
                <div class="bg-info bg-opacity-10 p-3 rounded-circle me-3 d-flex align-items-center justify-content-center" style="color: #8b5cf6; width: 48px; height: 48px;"><i class="bi bi-headset fs-5"></i></div>
                <div>
